...
初期移行では、ルーターと Kubernetes ノードが既に同じ L2 で到達できる
既存 subnet 上のアドレスを共有公開 VIP として使う。
...
| アドレス | 用途 |
| --- | --- |
| `172.16.2.30` | 共有公開 IPv4 DMZ VIP |
| `172.16.3.64/26` | 将来の非共有 IPv4 LoadBalancer pool |

IPv6 も初期移行では、ルーターと Kubernetes ノードが既に同じ L2 で到達できる
`240b:10:3f6d:1402::/64` 上のアドレスを共有公開 VIP として使う。

初期割り当て案:

| アドレス | 用途 |
| --- | --- |
| `240b:10:3f6d:1402::10` | 共有公開 IPv6 DMZ VIP |
| `240b:10:3f6d:1402::/112` | 将来の非共有 IPv6 LoadBalancer pool |
...
```text
global IPv4:80/tcp    -> 172.16.2.30:80/tcp
global IPv4:443/tcp   -> 172.16.2.30:443/tcp
global IPv4:443/udp   -> 172.16.2.30:443/udp
global IPv4:9987/udp  -> 172.16.2.30:9987/udp
global IPv4:10011/tcp -> 172.16.2.30:10011/tcp
global IPv4:30033/tcp -> 172.16.2.30:30033/tcp
```
...
```text
global IPv4:port -> 172.16.2.30:port
```

L2 案との違いは、ルーターが `172.16.2.30` へ到達する方法。L2 案では
LAN-local VIP として ARP で解決するが、BGP 案では Cilium から
`172.16.2.30/32` を BGP で学習する。
...
`172.16.2.30` と IPv6 アドレス `240b:10:3f6d:1402::10` を共有できる。
...
`172.16.2.30` を公開してはいけない。public `A` record はルーターの
...
2. `172.16.2.30` を Kubernetes 公開 LoadBalancer アドレス用に予約する。
3. 次の IPv6 `/64` を予約する。この文書では
   `240b:10:3f6d:1402::/64` としている。
...
10. ルーターの IPv4 DNAT 先を `172.16.2.30` に変更する。
...
```sh
ip neigh show 172.16.2.30
ip -6 neigh show 240b:10:3f6d:1402::10
```

BGP 案では、ルーターが VIP route を学習していることを確認する。

```sh
ip route get 172.16.2.30
ip -6 route get 240b:10:3f6d:1402::10
```
