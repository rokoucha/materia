...
本番移行では、ルーターと Kubernetes ノードが VLAN 30 で同じ L2 に参加する
公開 LoadBalancer 用 subnet 上のアドレスを共有公開 VIP として使う。
...
IPv6 も VLAN 30 上の `240b:10:3f6d:1403::/64` を公開 LoadBalancer 用 prefix
として使う。
